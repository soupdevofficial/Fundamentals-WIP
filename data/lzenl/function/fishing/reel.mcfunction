...
tag @s add user
execute at @s as @e[tag=caught,tag=fishing,type=armor_stand,distance=..40] if score @s .id = @e[type=interaction,distance=0,limit=1] .id at @s run tp @s ^ ^ ^1 facing entity @n[type=interaction,tag=user,distance=..50,limit=1]
tag @s remove user
