...
give @p[tag=user,distance=..7] gold_nugget[item_name="Enchanting Quill",enchantment_glint_override=true,rarity=rare,damage=0,max_damage=10,max_stack_size=1,item_model=spectral_arrow]

execute if function lzenl:scroll_table/recipe/repair_ench_quill2 run item replace block ~ ~-1 ~ container.16 with stone_button[item_name=execute,item_model="spectral_arrow",enchantment_glint_override=true,custom_name={text:"Enchanting Quill",italic:false},rarity=rare,custom_data={result:1b,run:'lzenl:scroll_table/return/quill_repair2'},max_stack_size=1,damage=0,max_damage=10]
